...
public class BloomFilteringMapperHTable extends
	...
	public void map(Object key, Text value, Context context)
		...
		String userid = parsed.get("LastEditorUserId");
		...
		if (filter.membershipTest(new Key(userid.getBytes()))) {
			...
			if (r!=null){
				byte[] rep = r.getValue("data".getBytes(), "reputation".getBytes());
				if (rep!=null){
					int reputation = Integer.parseInt(new String(rep));
					...
					context.write(new Text("WRONG VALUE: user: "+userid), NullWritable.get());
				}
			}
		}
	}
