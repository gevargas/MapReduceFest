...
public class CartesianRecordReader<K1, V1, K2, V2> implements RecordReader<Text, Text>
{
	...
	public CartesianRecordReader(CompositeInputSplit split, JobConf conf, Reporter reporter) throws IOException
	{
		...
		FileInputFormat leftFIF = Utils.newFileInputFormat(conf.get(CartesianInputFormat.LEFT_INPUT_FORMAT), conf);
		leftRR = leftFIF.getRecordReader(split.get(0), conf, reporter);

		// Create right record reader
		rightFIF = Utils.newFileInputFormat(conf.get(CartesianInputFormat.RIGHT_INPUT_FORMAT), conf);
	...
	public boolean next(Text key, Text value) throws IOException
	{
		do
		{
			...
			// This loop will continue if we finished reading key/value pairs from the right data set
		} while (goToNextLeft);
